...
const SPEND_COLS = ["spend", "amount_spent", "cost"];
...
async function upsertTikTokSpend(
  projectId: string,
  dailyRows: { project_id: string; ts: string; tiktok_spend: number }[]
): Promise<void> {
  const supabase = getSupabaseServiceClient();
  const batchSize = 500;
...
export async function POST(
  ...
  try {
    ...
    if (!spendCol) {
      return NextResponse.json(
        { status: "error", message: "No spend column found. Expected one of: spend, amount_spent, cost" },
        { status: 400 }
      );
    }
    ...
    if (totalSpend === 0) {
      warnings.push("Total TikTok spend is $0 across all included rows.");
    }
    if (includedRows === 0) {
    ...
    const dailyRows = sortedDates.map((date) => ({
      ...
      ts: date,
      tiktok_spend: Math.round((dailySpend.get(date) ?? 0) * 100) / 100,
    }));

    await upsertTikTokSpend(projectId, dailyRows);
    ...
    console.error("TikTok Ads upload error:", error);
    return NextResponse.json(
      {
        status: "error",
