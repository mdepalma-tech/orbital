...
export function TikTokAdsUploadSection({ projectId }: { projectId: string }) {
  ...
  async function handleParse(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        `/api/projects/${projectId}/upload-tiktok-ads`,
    ...
    <div className="space-y-4">
      <form ref={formRef} onSubmit={handleParse} className="space-y-4">
        <div>
          <label className="block mb-2">
            <span className="text-sm text-gray-400 font-light">
              TikTok Ads Campaign Export CSV
            </span>
            <div className="mt-2">
              <input
                type="file"
                ...
                className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-lg text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-light file:bg-pink-500 file:text-white hover:file:bg-pink-600 file:cursor-pointer focus:border-pink-500/50 focus:outline-none font-light disabled:opacity-50"
              />
            </div>
            <p className="mt-2 text-xs text-gray-500 font-light">
              Campaign-level export segmented by day. Requires: date/day, spend or amount_spent.
            </p>
          </label>
        </div>
        ...
        {(stage === "upload" || stage === "error") && (
          <button
            ...
            className="w-full px-8 py-4 border border-pink-500/50 bg-pink-500/10 text-pink-300 rounded-lg font-light tracking-wide hover:bg-pink-500/20 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Parse & Preview CSV
          </button>
        )}
      ...
      {stage === "parsing" && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p className="text-gray-400 font-light animate-pulse">Parsing TikTok Ads CSV...</p>
        </div>
      )}
      ...
      {stage === "preview" && summary && (
        <div className="p-6 rounded-xl border border-pink-500/30 bg-pink-500/5 space-y-5">
          <div className="flex items-center gap-2 text-pink-400">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
            <span className="text-lg font-light">TikTok Ads Parse Summary</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="p-3 rounded-lg bg-black/30 border border-white/5">
              <p className="text-gray-500 text-xs font-light mb-1">Total TikTok Spend</p>
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              disabled={!canSave}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-pink-500 to-rose-500 rounded-lg font-light tracking-wide hover:shadow-lg hover:shadow-pink-500/50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Confirm & Save
            </button>
            <button
      ...
      {stage === "saved" && summary && (
        <div className="p-6 rounded-xl border border-emerald-500/30 bg-emerald-500/5 space-y-4">
          <div className="flex items-center gap-2 text-emerald-400">
            ...
            <span className="text-lg font-light">TikTok Spend Saved Successfully</span>
          </div>
          ...
          <div className="p-3 rounded-lg bg-pink-500/10 border border-pink-500/30">
            <p className="text-sm text-pink-300 font-light">
              TikTok Ads spend has been stored in the tiktok_spend column. You can combine with other channel uploads or run your causal analysis.
            </p>
          </div>
      ...
      {stage === "error" && error && (
        <div className="p-6 rounded-xl border border-red-500/30 bg-red-500/5 space-y-3">
          ...
          <div className="p-3 rounded-lg bg-black/20 text-xs text-gray-400 font-light">
            <p className="font-medium text-gray-300 mb-2">Expected TikTok Ads CSV format:</p>
            <ul className="list-disc list-inside space-y-1">
              <li>Required: date, day, or date_start</li>
              <li>Required: spend, amount_spent, or cost</li>
